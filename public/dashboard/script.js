...
function renderNotes() {
    ...
    for (let i = 0; i < notes.length; i++) {
        ...
        let time=notes[i].created_at.substring(11, 12)>=12?'AM':'PM'
        let date = notes[i].created_at.substring(0, 10) + "   " + notes[i].created_at.substring(11, 16)+" "+time;
...
function filterNotes(searchTerm) {
    ...
    for (let i = 0; i < filteredNotes.length; i++) {
        let time=notes[i].created_at.substring(11, 12)>=12?'AM':'PM'
        let date = filteredNotes[i].created_at.substring(0, 10) + "   " + filteredNotes[i].created_at.substring(12, 16)+" "+time;
...
const displayComments = async () => {
    ...
    axios.get(`http://localhost:3000/messages/note_id/${notes[activeNoteIndex].note_id}`)
        .then(response => {
            ...
            fetchedMessages.forEach(message => {
                ...
                let time=commentDate.substring(11, 12)>=12?'AM':'PM'
                let formattedDate = `${commentDate.substring(0, 10)}  ${commentDate.substring(11, 16)} ${time}`;
    ...
    function filterMessages(searchTerm) {
        ...
        for (let i = 0; i < filteredMessages.length; i++) {
             let time=filteredMessages[i].sent_at.substring(11, 12)>=12?'AM':'PM'
            messageHTML += `
            <div class="comment-container" style="display: flex; align-items: flex-start; margin-bottom: 10px;">
                       <div class="comment" style="max-width: 500px; min-width:200px;border: 1px solid #ccc; background-color: #f9f9f9; padding: 10px; margin-bottom: 10px; display: flex;">
                             <div class="comment-content" style="display: flex; flex-direction: column;">
                                <div class="comment-text" style="word-wrap: break-word;">${filteredMessages[i].message_content}</div><br>
                                   <div class="comment-date" style="margin-top: 5px;display:flex;justify-content:space-between;min-width:200px">${filteredMessages[i].sent_at.substring(0, 10) + "  " + filteredMessages[i].sent_at.substring(11, 16)+" "+ time} 
